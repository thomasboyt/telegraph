...
export class Sync<T> {
  ...
  constructor(
    ...
    this.savedStates = {
      head: 0,
      frames: new Array(this.maxPredictionFrames + 2), // why we add + 2 to this, I am unclear, but it's required or things break!
    };
  ...
  getInRollback(): boolean { return this.inRollback; }
  ...
  addLocalInput(queueIdx: number, inputs: InputValues): GameInput | null {
    ...
    if (this.frameCount === 0) { this.saveCurrentFrame(); }
  ...
  // TODO(StoneCypher): extract methods up
  ...
  getSynchronizedInputs(): SyncInputResultValue {
    ...
    return {
      ...
      inputs:
        this.inputQueues.map((queue, i) =>
          isDisconnected(i)
            ? []
            : queue.getInput(this.frameCount).inputs),

      disconnected:
        this.inputQueues.map((queue, i) => isDisconnected(i))
  ...
  checkSimulation(): void {
    ...
    if (incorrectFrame !== -1) { this.adjustSimulation(incorrectFrame); }
  }
  ...
  adjustSimulation(seekTo: number): void {
    ...
    for (let i = 0; i < count; ++i) { this.callbacks.onAdvanceFrame(); }
  ...
  private loadFrame(frame: number): void {
    ...
    if (frame === this.frameCount) { return; } // already loaded!
  ...
  getLastSavedFrame(): SavedFrame<T> {
    ...
    if (i < 0) { i = this.savedStates.frames.length - 1; }
